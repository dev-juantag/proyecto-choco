...
export async function GET(
  ...
  try {
    ...
    const ficha = await (prisma.fichaHogar as any).findUnique({
      ...
      include: {
        ...
        compromisos: {
          include: {
            paciente: true,
            creadoEn: { select: { consecutivo: true, createdAt: true } },
            verificadoEn: { select: { consecutivo: true, createdAt: true } }
          },
    ...
    if (!ficha.familiogramaCodigo && ficha.pacientes && ficha.pacientes.length > 0) {
       ...
       ficha.familiogramaCodigo = generateFamiliogramaAutoLayout(
         ficha.pacientes.map((p: any) => ({ ...p, numDoc: p.documento }))
       );
    }
...
export async function DELETE(
  ...
  try {
    ...
    // Ejecutar todas las eliminaciones en una única transacción
    await prisma.$transaction([
      // 1. Eliminar derivaciones relacionadas a esta ficha
      prisma.derivacion.deleteMany({
        where: { fichaId: id }
      }),
