...
export function AtencionesModule() {
  ...
  useEffect(() => {
    setPage(1)
  }, [search, filterPrograma, filterTime])
              ...
              {errAt || errSt || errPr ? (
                ...
                (() => {
                  ...
                  const currentAtenciones = isAdmin && currentStageStart 
                    ? paginatedFiltered.filter(a => new Date(a.createdAtISO || (a.fecha + "T00:00:00")) >= limitDate)
                    : paginatedFiltered;
        ...
        {!loading && totalPages > 1 && (
          <div className="p-4 border-t border-border bg-muted/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground font-medium">
              Mostrando página <span className="font-bold text-foreground">{page}</span> de <span className="font-bold text-foreground">{totalPages}</span>
              {' '}(Total: {filtered.length} atenciones)
            </p>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="px-4 py-2 border border-border bg-background hover:bg-muted text-sm font-semibold rounded-lg transition-colors disabled:opacity-50 cursor-pointer"
              >
                Anterior
              </button>
              <button
                ...
                {fechaNacimiento && (
                  <div className="text-xs font-semibold text-[#0a8c32] bg-[#0a8c32]/10 px-3 py-1.5 rounded-md inline-block">
                    {calcularEdad(fechaNacimiento) !== null && (
                      <>
                        {calcularEdad(fechaNacimiento)} años · {calcularCursoVida(fechaNacimiento)}
                      </>
                    )}
                  </div>
                )}
        ...
        <FormSection icon={<User className="h-5 w-5" />} title="Datos del Paciente">
          ...
          <div className="grid gap-4 mt-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {atencion.pacienteFechaNac && calcularEdad(atencion.pacienteFechaNac) !== null && (
              <DetailItem 
                label="Curso de Vida" 
                value={calcularCursoVida(atencion.pacienteFechaNac)} 
              />
            )}
